...
public class MainNoNiceII {
    ...
    static Graph generateNoNiceII() {
        Random rnd = new Random();
        Graph g = new Graph(10);

        // Aristas base del Petersen
        for (int[] e : petersenEdges)
            g.addEdge(e[0], e[1], 10 + rnd.nextInt(91));

        // Añadir 2 pares de aristas antiparalelas (no-nice)
        g.addEdge(0, 1, 40 + rnd.nextInt(60)); // ya existe 0→1
        g.addEdge(1, 0, 10 + rnd.nextInt(50)); // antiparalela

        g.addEdge(3, 4, 20 + rnd.nextInt(80)); // ya existe 3→4
        g.addEdge(4, 3, 5 + rnd.nextInt(30));  // antiparalela

        return g;
    }

    public static void main(String[] args) {
        Graph g1 = generateNoNiceII();
        Graph g2 = generateNoNiceII();
        ...
        runBoth(g1, "Red 1 (no-nice II)");
        runBoth(g2, "Red 2 (no-nice II)");
